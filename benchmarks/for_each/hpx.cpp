...
std::chrono::nanoseconds measure_time_hpx(size_t num_threads) {
  ...
  auto beg = std::chrono::high_resolution_clock::now();
    ...
    hpx::execution::static_chunk_size cs(vec.size()/hpx::get_num_worker_threads());
    hpx::for_each(hpx::execution::par.with(cs), vec.begin(), vec.end(), 
        [](auto& v){
            v = std::tan(v);
        }
    );
  ...
  no_opt = hpx::count(hpx::execution::par, vec.begin(), vec.end(), rand());
  return std::chrono::duration_cast<std::chrono::nanoseconds>(end - beg);
}

std::chrono::nanoseconds measure_time_hpx_simd(size_t num_threads) {
  hpx::execution::static_chunk_size cs(vec.size()/hpx::get_num_worker_threads());
  auto beg = std::chrono::high_resolution_clock::now();
    hpx::for_each(hpx::execution::par_simd.with(cs), vec.begin(), vec.end(), 
        [](auto& v){
            v = eve::tan(v);
        }
    );
  ...
  for(size_t N=std::pow(2, 4); N<=std::pow(2, 25); N = N*2) {
    ...
    for(unsigned j=0; j<num_rounds; ++j) {
     ...
     if(model == "hpx") {
        ...
        runtime += measure_time_hpx_simd(num_threads).count();
      }
      else assert(false);
    }
